...
def NSGA2(population_individuals: List[Individual], offspring: List[Individual], debug: bool = False):
    ...
    objectives = np.zeros(
        (population_size + offspring_size, max(1, len(population_individuals[0]))))  # TODO fitnesses is 0
    ...
    for index, individual in enumerate(all_individuals):
        ...
        for ind, k in enumerate(individual.keys()):
            objectives[index, ind] = (inf if individual[k].fitness is None else -individual[k].fitness)
...
def _visualize(objectives, sorted_fronts, new_individuals, discarded_population):
    ...
    for individual in new_individuals:
        obj = np.zeros((1, 2))
        for ind, k in enumerate(individual.keys()):
            obj[0, ind] = (inf if individual[k].fitness is None else individual[k].fitness)
        plt.scatter(obj[0,0], obj[0,1], s=5, color='black')
    for individual in discarded_population:
        for ind, k in enumerate(individual.keys()):
            obj[0, ind] = (inf if individual[k].fitness is None else individual[k].fitness)
        plt.scatter(obj[0, 0], obj[0, 1], s=5, color='red')
