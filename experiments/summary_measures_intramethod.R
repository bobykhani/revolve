...
base_directory <-paste('../1/', sep='')
analysis = 'results'
output_directory = paste(base_directory,'',analysis ,sep='')

experiments_type = c('default_experiment')
  ...
  for(run in runs[[exp]])
  {
    for (env in 1:length(environments[[exp]]))
    {
      input_directory  <-  paste(base_directory, '/', 
                                 experiments_type[exp], '',sep='')
      
      measures   = read.table(paste(input_directory, '/data_fullevolution/',
      ...
      snapshots   = read.table(paste(input_directory,'/selectedpop_',
