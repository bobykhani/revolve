...
for(m in 1:length(paths))
{
    ...
    for (exp in runs[[m]])
    	
      {
        ...
        bests =  sqldf(paste("select robot_id, displacement_velocity_hill from measures inner join ids_gens using (robot_id) order by displacement_velocity_hill desc limit",num_top))
      ...
      #  bests =  sqldf(paste("select robot_id, cons_fitness from measures inner join ids_gens using (robot_id, generation) order by cons_fitness desc limit",num_top))
        ...
        for(b in 1:nrow(bests))
        {
           ...
           writeLines( paste(paths[m],'exp',exp,bests[b,'robot_id'] ,bests[b,'displacement_velocity_hill'] ), file )
          print( paste(paths[m],'exp',exp,bests[b,'robot_id'] ,bests[b,'displacement_velocity_hill'] ))
