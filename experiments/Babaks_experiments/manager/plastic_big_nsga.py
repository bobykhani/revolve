...
from pyrevolve.evolution.nsga2 import NSGA2
...
async def run():
    ...
    if do_recovery:
        ...
        gen_num = 0
        next_robot_id = 1
    def fitness_function_plane_nsga(robot_manager, robot):
        return fitness.displacement_velocity_hill_No_penalty(robot_manager, robot, False)

    def fitness_function_plane(robot_manager, robot):
        return fitness.displacement_velocity_hill(robot_manager, robot, False)

    def fitness_function_dvh_c_5050(robot_manager, robot):
        return fitness.dvh_c_5050(robot_manager, robot, False)

    def fitness_function_dvh_c_b_33(robot_manager, robot):
        return fitness.dvh_c_b_33(robot_manager, robot, False)
...
#                        'plane': fitness_function_dvh_c_b_701515}#
    ...
    population_conf = PopulationConfig(
        ...
        population_management=lambda pop, offsprings: NSGA2(pop, offsprings),#steady_state_population_management,
        population_management_selector=None,#tournament_selection,
    ...
    # output result after completing all generations...
        ...
        if not os.path.exists(experiment_management.dirpath+"/nsga_plot"):
            os.makedirs(experiment_management.dirpath+"/nsga_plot")
        generation_folder = experiment_management.dirpath+"/nsga_plot"##experiment_management.generation_folder(gen_num)
