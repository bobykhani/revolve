...
environments = list(
                    ...
                    c('plane')
                    #( 'plane_g20')
                    #,c( 'plane_directional_12')
                    #,c( 'plane_directional_21')
                   # c( 'tilted5_g5'),
                    #c('tilted5_g20')
                    #,c('tilted5_directional_12')
                    ) # update with desired environment
...
measures_names = c(
  ...
  'DisplacementSum',
  'x_axis_displacement',
  'displacement_full_avg',
  'gaitAngleErrorCumulative',
  'EffectiveMovement',
  'DifFromIdealMovement',
  'DifFromIdealMovementX',
  'DifFromIdealMovementY',
  'StandingHeight'
)
...
for (exp in 1:length(experiments_type))
{
  for(run in runs[[exp]])
  {
    for (env in 1:length(environments[[exp]]))
    {
      ...
      measures = read.table(paste(input_directory, run, '/data_fullevolution/',environments[[exp]][env], "/all_measures.tsv", sep=''), header = TRUE, fill=TRUE)
      ...
      #                         header = TRUE)
      snapshots = read.table(paste(input_directory, run,'/selectedpop_',environments[[exp]][env],"/snapshots_ids.tsv", sep=''), header = TRUE)

      measures_snapshots = sqldf('select * from snapshots inner join measures using(robot_id) order by generation')
      measures_snapshots = sqldf("select * from measures_snapshots where generation <'100'")
      ...
      if ( is.null(measures_snapshots_all)){
        ...
        common_cols <- intersect(colnames(measures_snapshots_all), colnames(measures_snapshots))
        measures_snapshots_all = rbind(subset(measures_snapshots_all, select = common_cols),subset(measures_snapshots, select = common_cols))
      }
    }
  }
}
   ...
   for (out in outliers)
   {
     ...
     for(gc in gens_box_comparisons)
     {
          ...
          for (met in 1:length(methods))
          {
            ...
            colnames(temp) <- methods_labels[met]
